...
func validateManifestCommand(args []string, stdout, stderr io.Writer) int {
	...
	paths := flags.Args()
	if len(paths) == 0 {
		fmt.Fprintln(stderr, "usage: wendy-validate manifest <manifest-file-or-dir> [...]")
		return 2
	}
...
func validateInvokeCommand(kind string, args []string, stdout, stderr io.Writer) int {
	...
	if *manifestPath == "" || *capabilityID == "" || flags.NArg() != 1 {
		fmt.Fprintf(stderr, "usage: wendy-validate %s -manifest <manifest-file-or-dir> -capability <capability-id> <payload.json>\n", kind)
		return 2
	}
...
func usage(stderr io.Writer) int {
	fmt.Fprintln(stderr, "usage: wendy-validate <manifest|provider-invoke|tool-invoke> [options]")
