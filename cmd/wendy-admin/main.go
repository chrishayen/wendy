...
func run(args []string, stdout, stderr io.Writer, httpClient *http.Client) int {
	cfg := adminConfig{}
	flags := flag.NewFlagSet("wendy-admin", flag.ContinueOnError)
	flags.SetOutput(stderr)
	flags.StringVar(&cfg.CatalogURL, "catalog-url", envOrDefault("WENDY_CATALOG_URL", "http://localhost:18081"), "catalog service base URL")
	flags.StringVar(&cfg.JobsURL, "jobs-url", envOrDefault("WENDY_JOBS_URL", "http://localhost:18082"), "jobs service base URL")
	flags.StringVar(&cfg.LeasesURL, "leases-url", envOrDefault("WENDY_LEASES_URL", "http://localhost:18083"), "lease service base URL")
	flags.StringVar(&cfg.ArtifactsURL, "artifacts-url", envOrDefault("WENDY_ARTIFACTS_URL", "http://localhost:18084"), "artifact service base URL")
	flags.StringVar(&cfg.PolicyURL, "policy-url", envOrDefault("WENDY_POLICY_URL", "http://localhost:18085"), "policy service base URL")
	flags.StringVar(&cfg.GatewayURL, "gateway-url", envOrDefault("WENDY_GATEWAY_URL", "http://localhost:18086"), "gateway service base URL")
	flags.StringVar(&cfg.NodeRegistryURL, "node-registry-url", envOrDefault("WENDY_NODE_REGISTRY_URL", "http://localhost:18080"), "node registry service base URL")
	flags.StringVar(&cfg.NodeURL, "node-url", os.Getenv("WENDY_NODE_URL"), "optional node service base URL")
	flags.StringVar(&cfg.NodeURLs, "node-urls", os.Getenv("WENDY_NODE_URLS"), "optional comma-separated node_id=URL entries")
	flags.StringVar(&cfg.RunnerURL, "runner-url", os.Getenv("WENDY_RUNNER_URL"), "optional runner monitor base URL")
	flags.StringVar(&cfg.ComponentToken, "component-token", os.Getenv("WENDY_COMPONENT_TOKEN"), "optional component API bearer token or raw token")
	flags.StringVar(&cfg.GatewayToken, "gateway-token", envFirst("WENDY_GATEWAY_TOKEN", "WENDY_AGENT_TOKEN"), "optional gateway API bearer token or raw token")
	flags.StringVar(&cfg.NodeToken, "node-token", os.Getenv("WENDY_NODE_TOKEN"), "optional node API bearer token or raw token")
	flags.StringVar(&cfg.RunnerToken, "runner-token", os.Getenv("WENDY_RUNNER_MONITOR_TOKEN"), "optional runner monitor bearer token or raw token")
	flags.StringVar(&cfg.RequestID, "request-id", os.Getenv("WENDY_REQUEST_ID"), "optional request id propagated as X-Request-ID")
...
func healthCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] health [-providers]")
	}
...
func metricsCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] metrics [-providers]")
	}
...
func alertsCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] alerts [-providers] [-node-registry] [-queue-depth-threshold n] [-runner-heartbeat-stale-after duration]")
	}
...
func catalogCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "usage: wendy-admin [flags] catalog <services|service|capabilities|capability|route|tags|import> [id]")
		return 2
	}
	switch args[0] {
	case "import":
	...
	case "services":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] catalog services")
		}
		return getJSON(cfg, httpClient, cfg.CatalogURL, "/v1/catalog/services", authorizationHeader(cfg.ComponentToken), stdout, stderr)
	case "service":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] catalog service <service-id>")
		}
	...
	case "capabilities":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] catalog capabilities")
		}
	...
	case "capability":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] catalog capability <capability-id>")
		}
	...
	case "route":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] catalog route <capability-id>")
		}
	...
	case "tags":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] catalog tags")
		}
		return getJSON(cfg, httpClient, cfg.CatalogURL, "/v1/catalog/tags", authorizationHeader(cfg.ComponentToken), stdout, stderr)
	default:
		return usage(stderr, "usage: wendy-admin [flags] catalog <services|service|capabilities|capability|route|tags|import> [id]")
	}
}

func catalogImportCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] catalog import <manifest-file-or-dir>")
	}
...
func jobsCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return usage(stderr, "usage: wendy-admin [flags] jobs <list|job|logs|cancel> [id]")
	}
	switch args[0] {
	case "list":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] jobs list")
		}
		return getJSON(cfg, httpClient, cfg.JobsURL, "/v1/jobs", authorizationHeader(cfg.ComponentToken), stdout, stderr)
	case "job":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] jobs job <job-id>")
		}
	...
	case "logs":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] jobs logs <job-id>")
		}
...
func jobsCancelCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] jobs cancel <job-id> -idempotency-key <key> [-reason text]")
	}
	if *idempotencyKey == "" {
		return usage(stderr, "idempotency-key is required for jobs cancel")
	}
	if cfg.GatewayToken == "" {
		return usage(stderr, "gateway-token is required for jobs cancel; set -gateway-token, WENDY_GATEWAY_TOKEN, or WENDY_AGENT_TOKEN")
	}
	body := map[string]any{}
	if *reason != "" {
		body["reason"] = *reason
...
func leasesCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return usage(stderr, "usage: wendy-admin [flags] leases <resources|resource|register-resource|inspect|requests|request|create-request|cancel-request|lease|release> [id]")
	}
	switch args[0] {
	case "resources":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] leases resources")
		}
		return getJSON(cfg, httpClient, cfg.LeasesURL, "/v1/resources", authorizationHeader(cfg.ComponentToken), stdout, stderr)
	case "resource":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] leases resource <resource-id>")
		}
	...
	case "inspect":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] leases inspect <resource-id>")
		}
	...
	case "request":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] leases request <request-id>")
		}
	...
	case "lease":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] leases lease <lease-id>")
		}
...
func leasesRegisterResourceCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] leases register-resource -selector <selector> [-resource-id id] [-node-id node] [-tags a,b] [-metadata JSON]")
	}
	if *selector == "" {
...
func leasesRequestsCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] leases requests -requester-id <id>")
	}
	if *requesterID == "" {
...
func leasesCreateRequestCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] leases create-request -requester-id <id> -selector <selector> [-priority n] [-heartbeat-timeout-seconds n]")
	}
	if *requesterID == "" {
...
func leasesCancelRequestCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] leases cancel-request <request-id> [-reason text]")
	}
	body := map[string]any{}
	if *reason != "" {
		body["reason"] = *reason
...
func leasesReleaseCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] leases release <lease-id> -holder-id <holder> -idempotency-key <key> [-reason text] [-actor-subject-id sub_admin]")
	}
	if *holderID == "" {
...
func artifactsCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return usage(stderr, "usage: wendy-admin [flags] artifacts <list|artifact|upload|create-upload|put-content|complete-upload|register-local|retention-sweep> [id]")
	}
	switch args[0] {
	case "list":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] artifacts list")
		}
		return getJSON(cfg, httpClient, cfg.ArtifactsURL, "/v1/artifacts", authorizationHeader(cfg.ComponentToken), stdout, stderr)
	case "artifact":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] artifacts artifact <artifact-id>")
		}
	...
	case "upload":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] artifacts upload <upload-id>")
		}
	...
	case "retention-sweep":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] artifacts retention-sweep")
		}
...
func artifactsCreateUploadCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] artifacts create-upload -name <name> -media-type <type> -owner-subject-id <subject> -idempotency-key <key> [-producer-ref ref] [-expected-size bytes] [-expected-checksum sha256:hex] [-metadata JSON]")
	}
	if *name == "" {
...
func artifactsPutContentCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] artifacts put-content <upload-id> -file <path> -media-type <type> -idempotency-key <key> [-digest sha256:hex]")
	}
	if *filePath == "" {
...
func artifactsCompleteUploadCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] artifacts complete-upload <upload-id> -idempotency-key <key> [-file path] [-checksum sha256:hex -size bytes]")
	}
...
func artifactsRegisterLocalCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] artifacts register-local -path <store-relative-path> -media-type <type> -owner-subject-id <subject> [-name name] [-producer-ref ref] [-metadata JSON]")
	}
	if *pathFlag == "" {
...
func policyCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy <create-key|rotate-key|revoke-key|verify|check|create-rule|audit-events|create-secret|redact>")
	}
	switch args[0] {
	case "create-key":
	...
	case "revoke-key":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] policy revoke-key <key-id>")
		}
...
func policyCreateKeyCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy create-key -subject-id <id> -scopes <scope[,scope]> [-token token]")
	}
	if *subjectID == "" {
...
func policyRotateKeyCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] policy rotate-key <key-id> [-token token]")
	}
...
func policyVerifyCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy verify -credential 'Bearer <token>'")
	}
	if *credential == "" {
...
func policyCheckCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy check -subject-id <id> -action <action> -resource <resource> [-context JSON]")
	}
	if *subjectID == "" {
...
func policyCreateRuleCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy create-rule [-subject-id id|-scope scope] -action <action> -resource <resource> -effect <allow|deny> [-reason text]")
	}
	if *subjectID == "" && *scope == "" {
...
func policyAuditEventsCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy audit-events [-limit n] [-cursor cursor]")
	}
	if *limit < 0 {
...
func policyCreateSecretCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy create-secret -name <name> (-value <secret>|-value-env ENV)")
	}
	if *name == "" {
...
func policyRedactCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] policy redact -text <text>")
	}
	if *text == "" {
...
func nodeRegistryCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return usage(stderr, "usage: wendy-admin [flags] node-registry <list|get|register|trust|heartbeat> [id]")
	}
	switch args[0] {
	case "list":
	...
	case "get":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] node-registry get <node-id>")
		}
...
func nodeRegistryListCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 0 {
		return usage(stderr, "usage: wendy-admin [flags] node-registry list [-limit n] [-cursor cursor]")
	}
	query := url.Values{}
	if *limit > 0 {
		query.Set("limit", strconv.Itoa(*limit))
...
func nodeRegistryRegisterCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] node-registry register <node-id> -url <url> [-display-name name] [-trust-state state] [-status status] [-tags a,b] [-metadata JSON]")
	}
...
func nodeRegistryTrustCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] node-registry trust <node-id> -trust-state <trusted|untrusted|disabled> [-reason text]")
	}
...
func nodeRegistryHeartbeatCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] node-registry heartbeat <node-id> -status <registered|reachable|unreachable> [-metadata JSON]")
	}
...
func nodeCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if cfg.NodeURL == "" {
		fmt.Fprintln(stderr, "node-url is required; set -node-url or WENDY_NODE_URL")
		return 2
	}
	if len(args) == 0 {
		return usage(stderr, "usage: wendy-admin [flags] node <resources|services|service|events|start|touch|stop> [id]")
	}
	switch args[0] {
	case "resources":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] node resources")
		}
		return getJSON(cfg, httpClient, cfg.NodeURL, "/v1/node/resources", authorizationHeader(cfg.NodeToken), stdout, stderr)
	case "services":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] node services")
		}
		return getJSON(cfg, httpClient, cfg.NodeURL, "/v1/node/services", authorizationHeader(cfg.NodeToken), stdout, stderr)
	case "service":
		if len(args) != 2 {
			return usage(stderr, "usage: wendy-admin [flags] node service <service-id>")
		}
	...
	case "events":
		if len(args) != 1 {
			return usage(stderr, "usage: wendy-admin [flags] node events")
		}
...
func nodeStartCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] node start <service-id> -idempotency-key <key>")
	}
	if *idempotencyKey == "" {
...
func nodeTouchCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] node touch <service-id>")
	}
...
func nodeStopCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	...
	if len(remaining) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] node stop <service-id> -idempotency-key <key>")
	}
	if *idempotencyKey == "" {
...
func diagnoseCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return usage(stderr, "usage: wendy-admin [flags] diagnose <job|resource> [id]")
	}
	switch args[0] {
	case "job":
...
func diagnoseJobCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] diagnose job <job-id>")
	}
	jobID := args[0]
...
func diagnoseResourceCommand(cfg adminConfig, httpClient *http.Client, args []string, stdout, stderr io.Writer) int {
	if len(args) != 1 {
		return usage(stderr, "usage: wendy-admin [flags] diagnose resource <resource-id>")
	}
	resourceID := args[0]
...
func printUsage(w io.Writer) {
	fmt.Fprintln(w, "usage: wendy-admin [flags] <command>")
