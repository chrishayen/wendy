...
func main() {
	...
	policyURL := flag.String("policy-url", os.Getenv("WENDY_POLICY_URL"), "optional policy service base URL for route-aware lease API auth")
	policyCredential := flag.String("policy-credential", componentCredentialDefault("WENDY_LEASES_POLICY_CREDENTIAL"), "optional credential used when calling policy auth verify; defaults to WENDY_LEASES_POLICY_CREDENTIAL or WENDY_COMPONENT_TOKEN")
	flag.Parse()
