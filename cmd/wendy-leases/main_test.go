...
func TestLeasesPolicyCredentialDefault(t *testing.T) {
	t.Setenv("WENDY_LEASES_POLICY_CREDENTIAL", "leases-policy-token")
	t.Setenv("WENDY_COMPONENT_TOKEN", "component-token")

	if got := componentCredentialDefault("WENDY_LEASES_POLICY_CREDENTIAL"); got != "leases-policy-token" {
...
func TestLeasesPolicyCredentialDefaultFallsBackToComponentToken(t *testing.T) {
	...
	if got := componentCredentialDefault("WENDY_LEASES_POLICY_CREDENTIAL"); got != "component-token" {
		t.Fatalf("credential default = %q", got)
	}
}
