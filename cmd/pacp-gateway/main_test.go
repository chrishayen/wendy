...
func TestGatewayCredentialDefaultPrefersGatewayCredential(t *testing.T) {
	t.Setenv("PACP_GATEWAY_CREDENTIAL", "gateway-token")
	t.Setenv("PACP_COMPONENT_TOKEN", "component-token")

	if got := componentCredentialDefault("PACP_GATEWAY_CREDENTIAL"); got != "gateway-token" {
...
func TestGatewayCredentialDefaultFallsBackToComponentToken(t *testing.T) {
	...
	if got := componentCredentialDefault("PACP_GATEWAY_CREDENTIAL"); got != "component-token" {
		t.Fatalf("credential default = %q", got)
	}
}
