...
func TestArtifactsPolicyCredentialDefault(t *testing.T) {
	t.Setenv("PACP_ARTIFACTS_POLICY_CREDENTIAL", "artifacts-policy-token")
	t.Setenv("PACP_COMPONENT_TOKEN", "component-token")

	if got := componentCredentialDefault("PACP_ARTIFACTS_POLICY_CREDENTIAL"); got != "artifacts-policy-token" {
...
func TestArtifactsPolicyCredentialDefaultFallsBackToComponentToken(t *testing.T) {
	...
	if got := componentCredentialDefault("PACP_ARTIFACTS_POLICY_CREDENTIAL"); got != "component-token" {
		t.Fatalf("credential default = %q", got)
	}
}
