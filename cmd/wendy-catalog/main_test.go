...
func TestCatalogPolicyCredentialDefault(t *testing.T) {
	t.Setenv("WENDY_CATALOG_POLICY_CREDENTIAL", "catalog-policy-token")
	t.Setenv("WENDY_COMPONENT_TOKEN", "component-token")

	if got := componentCredentialDefault("WENDY_CATALOG_POLICY_CREDENTIAL"); got != "catalog-policy-token" {
...
func TestCatalogPolicyCredentialDefaultFallsBackToComponentToken(t *testing.T) {
	...
	if got := componentCredentialDefault("WENDY_CATALOG_POLICY_CREDENTIAL"); got != "component-token" {
		t.Fatalf("credential default = %q", got)
	}
}
