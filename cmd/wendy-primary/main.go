...
func main() {
	...
	flag.StringVar(&cfg.ArtifactRoot, "artifact-root", "/tmp/wendy-artifacts", "artifact storage root")
	...
	flag.StringVar(&cfg.ComponentToken, "component-token", os.Getenv("WENDY_COMPONENT_TOKEN"), "optional bearer token required for component API calls")
	flag.StringVar(&cfg.GatewayCredential, "gateway-credential", componentCredentialDefault("WENDY_GATEWAY_CREDENTIAL"), "component credential used by the gateway for downstream calls")
	flag.StringVar(&cfg.RunnerCredential, "runner-credential", componentCredentialDefault("WENDY_RUNNER_CREDENTIAL"), "component credential used by the runner for downstream calls")
	flag.StringVar(&cfg.RunnerPolicyCredential, "runner-policy-credential", componentCredentialDefault("WENDY_RUNNER_POLICY_CREDENTIAL"), "component credential used by the runner for policy service calls")
	flag.StringVar(&cfg.RunnerNodeRegistryCredential, "runner-node-registry-credential", componentCredentialDefault("WENDY_RUNNER_NODE_REGISTRY_CREDENTIAL"), "component credential used by the runner for node registry service calls")
	flag.StringVar(&cfg.RunnerSubjectID, "runner-subject-id", os.Getenv("WENDY_RUNNER_SUBJECT_ID"), "optional runner subject id for policy checks")
	flag.StringVar(&cfg.RunnerActorID, "runner-actor-subject-id", os.Getenv("WENDY_RUNNER_ACTOR_SUBJECT_ID"), "optional runner actor subject id for lease release audit; defaults to runner subject id")
	flag.StringVar(&cfg.WorkerID, "worker-id", "runner_primary", "runner worker id")
	flag.StringVar(&cfg.NodeURL, "node-url", os.Getenv("WENDY_NODE_URL"), "optional default node service base URL")
	flag.StringVar(&cfg.NodeURLsRaw, "node-urls", os.Getenv("WENDY_NODE_URLS"), "optional comma-separated node_id=URL mappings")
	flag.StringVar(&cfg.NodeRegistryURL, "node-registry-url", os.Getenv("WENDY_NODE_REGISTRY_URL"), "optional external node registry URL; defaults to the embedded registry service")
