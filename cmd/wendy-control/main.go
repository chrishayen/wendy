...
func run(args []string, stdout, stderr io.Writer, httpClient *http.Client) int {
	global := flag.NewFlagSet("wendy-control", flag.ContinueOnError)
	global.SetOutput(stderr)
	gatewayURL := global.String("gateway-url", os.Getenv("WENDY_GATEWAY_URL"), "gateway service base URL")
	token := global.String("token", os.Getenv("WENDY_AGENT_TOKEN"), "agent bearer token or raw token")
	requestID := global.String("request-id", os.Getenv("WENDY_REQUEST_ID"), "optional request id propagated as X-Request-ID")
	...
	if *gatewayURL == "" {
		fmt.Fprintln(stderr, "gateway-url is required; set -gateway-url or WENDY_GATEWAY_URL")
		return 2
	}
	if *token == "" && commandRequiresToken(remaining[0]) {
		fmt.Fprintln(stderr, "token is required; set -token or WENDY_AGENT_TOKEN")
		return 2
	}
	if httpClient == nil {
...
func runCommand(client gatewayClient, args []string, stdout, stderr io.Writer) (int, error) {
	...
	case "tool":
		if len(args) != 2 {
			return 2, errors.New("usage: wendy-control tool <capability-id>")
		}
	...
	case "job":
		if len(args) != 2 {
			return 2, errors.New("usage: wendy-control job <job-id>")
		}
	...
	case "queue":
		if len(args) != 2 {
			return 2, errors.New("usage: wendy-control queue <resource-selector>")
		}
...
func invokeCommand(client gatewayClient, args []string, stdout, stderr io.Writer) (int, error) {
	...
	if len(remaining) != 1 {
		return 2, errors.New("usage: wendy-control invoke <capability-id> -idempotency-key <key> [-input JSON] [-wait]")
	}
	if *idempotencyKey == "" {
...
func waitCommand(client gatewayClient, args []string, stdout, stderr io.Writer) (int, error) {
	...
	if len(remaining) != 1 {
		return 2, errors.New("usage: wendy-control wait <job-id> [-interval duration] [-timeout duration]")
	}
	if *interval <= 0 {
...
func cancelCommand(client gatewayClient, args []string, stdout, stderr io.Writer) (int, error) {
	...
	if len(remaining) != 1 {
		return 2, errors.New("usage: wendy-control cancel <job-id> -idempotency-key <key> [-reason text]")
	}
	if *idempotencyKey == "" {
...
func logsCommand(client gatewayClient, args []string, stdout, stderr io.Writer) (int, error) {
	...
	if len(remaining) != 1 {
		return 2, errors.New("usage: wendy-control logs <job-id> [-limit n] [-cursor cursor]")
	}
	query := url.Values{}
...
func artifactsCommand(client gatewayClient, args []string, stdout, stderr io.Writer) (int, error) {
	...
	if len(remaining) != 1 {
		return 2, errors.New("usage: wendy-control artifacts <job-id> [-out-dir dir]")
	}
	jobID := remaining[0]
...
func contentCommand(client gatewayClient, args []string, stdout, stderr io.Writer) (int, error) {
	...
	if len(remaining) != 1 {
		return 2, errors.New("usage: wendy-control artifact-content <artifact-id> [-out file]")
	}
...
func printUsage(w io.Writer) {
	fmt.Fprintln(w, "usage: wendy-control -gateway-url URL [-token TOKEN] <command> [args]")
