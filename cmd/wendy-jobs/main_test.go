...
func TestJobsPolicyCredentialDefault(t *testing.T) {
	t.Setenv("WENDY_JOBS_POLICY_CREDENTIAL", "jobs-policy-token")
	t.Setenv("WENDY_COMPONENT_TOKEN", "component-token")

	if got := componentCredentialDefault("WENDY_JOBS_POLICY_CREDENTIAL"); got != "jobs-policy-token" {
...
func TestJobsPolicyCredentialDefaultFallsBackToComponentToken(t *testing.T) {
	...
	if got := componentCredentialDefault("WENDY_JOBS_POLICY_CREDENTIAL"); got != "component-token" {
		t.Fatalf("credential default = %q", got)
	}
}
