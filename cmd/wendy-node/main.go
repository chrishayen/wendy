...
func main() {
	...
	nodeRegistryURL := flag.String("node-registry-url", os.Getenv("WENDY_NODE_REGISTRY_URL"), "optional node registry service base URL")
	nodeRegistryCredential := flag.String("node-registry-credential", envFirst("WENDY_NODE_REGISTRY_CREDENTIAL", "WENDY_COMPONENT_TOKEN"), "optional node registry bearer token or raw token")
	nodePublicURL := flag.String("node-public-url", os.Getenv("WENDY_NODE_PUBLIC_URL"), "public node API base URL used when registering this node")
	nodeRegistryRegister := flag.Bool("node-registry-register", boolEnv("WENDY_NODE_REGISTRY_REGISTER"), "register this node with the node registry on startup")
	nodeRegistryTrustState := flag.String("node-registry-trust-state", os.Getenv("WENDY_NODE_REGISTRY_TRUST_STATE"), "optional trust state to request when registering: trusted, untrusted, or disabled")
	nodeRegistryHeartbeat := flag.Duration("node-registry-heartbeat", durationEnvOrDefault("WENDY_NODE_REGISTRY_HEARTBEAT", 0), "optional interval for node registry reachability heartbeats; 0 disables")
	flag.Parse()
	if *configPath == "" {
