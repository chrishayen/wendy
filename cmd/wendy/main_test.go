...
func TestWriteConfigDoesNotOverwriteWithoutForce(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "wendy.yaml")
...
func TestLoadConfigExpandsEnvRefs(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "wendy.yaml")
	t.Setenv("WENDY_TEST_AGENT_TOKEN", "agent-from-env")
	raw := `
schema_version: v1
primary:
  host: localhost
credentials:
  agent: ${WENDY_TEST_AGENT_TOKEN}
...
func TestEnsureConfigCreatesConfigAndIgnore(t *testing.T) {
	...
	if !strings.Contains(string(ignore), "wendy.yaml") || !strings.Contains(string(ignore), ".wendy/") {
